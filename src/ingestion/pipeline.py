...
def ingest_document(file_path: str | Path, batch_size: int = 100, cleanup_after: bool = False, resume: bool = True) -> DocumentMetadata:
    ...
    try:
        ...
        log_ingestion(file_path=file_path, doc_id=metadata.doc_id, doc_type=metadata.doc_type, language=metadata.language,
            num_pages=page_count, num_chunks=metadata.num_chunks, status="resumed" if is_resumed else "completed",
        ...
        try:
            log_ingestion(file_path=file_path, doc_id=doc_id if doc_id else "unknown",
                ...
                language=metadata.language if metadata else "unknown", num_pages=page_count,
                num_chunks=len(chunks) if chunks else 0, status="failed", start_time=start_time, end_time=end_time)
        except Exception:
            pass
